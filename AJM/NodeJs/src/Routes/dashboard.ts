...
router.post('/MixerStatus', async (req,res,next) => {
var Mc=MongoClient.connect(mongoclienturl).then((db,err)=>{
   ...
   { var db0 = db.db("AJM");
    ...
    { console.log(err); next(ApiError.DbConnectError()); return;}
...
router.post('/HatchStatus', async (req,res,next) => {
    var Mc=MongoClient.connect(mongoclienturl).then((db,err)=>{
        if(!err)
       { var db0 = db.db("AJM");
    ...
    router.post('/LeverStatus', async (req,res,next) => {
    var Mc=MongoClient.connect(mongoclienturl).then((db,err)=>{
        if(!err)
       { var db0 = db.db("AJM");
        ...
        router.post('/TodaysCycleCount', async (req,res,next) => {
            var Mc=MongoClient.connect(mongoclienturl).then((db,err)=>{
                if(!err)
               { var db0 = db.db("AJM");
    ...
    router.post('/rpm1', async (req,res,next) => {
                var Mc=MongoClient.connect(mongoclienturl).then((db,err)=>{
                    if(!err)
                   { var db0 = db.db("AJM");
...
router.post('/rpm2', async (req,res,next) => {
        var Mc=MongoClient.connect(mongoclienturl).then((db,err)=>{
            if(!err)
           { var db0 = db.db("AJM");
...
router.post('/energyGraph', async (req,res,next) => {
    ...
    router.post('/CycleDetailsToday', async (req,res,next) => {
        var count=0
        var Mc=MongoClient.connect(mongoclienturl).then((db,err)=>{
            if(!err)
           { var db0 = db.db("AJM");
...
router.post('/TodayOnTimeTotal', async (req,res,next) => {
            var querydate=new Date(new Date().setUTCHours(0,0,0,0) )
                ...
                if(!err)
                   { var db0 = db.db("AJM");
                        ...
                        { $project: { dateDifference: {$divide:[ {$subtract: [ "$stoptime", "$time" ]},1000] },cycleCount:1}},
                         {$group:{_id:'', TimeDiffinMillis:{ $sum: '$dateDifference' }}}, //6656.037
