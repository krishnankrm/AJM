...
router.post('/MixerOnOffTime', async (req,res,next) => {
        ...
        if(!err)
           { var db0 = db.db("AJM");
                ...
                { $project: { dateDifference: {$divide:[ {$subtract: [ "$stoptime", "$time" ]},1000] }}},
                 {$group:{_id:'', TimeDiffinSec:{ $sum: '$dateDifference' }}}, //6656.037
                 ...
                 {$project:{Ontime:{$concat: [{$toString:"$hh"},':',{$toString:'$mm'},':',{$toString:'$ss'}]},OFF_time:{$concat: [{$toString:"$hh1"},':',{$toString:'$mm1'},':',{$toString:'$ss1'}]}}}
                    ...
                    if(!err)
                        ...
                        {    
                            console.log(err)
                            next(ApiError.DbQuerryError()); return;                           }
            })
        }
                ...
                {    
                    console.log(err)
                    next(ApiError.DbConnectError()); return;}
           })
})


router.post('/LatchOnOfftime', async (req,res,next) => {
    var StartDate=add530(req.body.start)
    var StopDate=add530(req.body.stop)
    MongoClient.connect(mongoclienturl,(err,db)=>{
        if(!err)
...
router.post('/HatchOnOfftime', async (req,res,next) => {
    var StartDate=add530(req.body.start)
    var StopDate=add530(req.body.stop)
...
router.post('/TotalCycleCount', async (req,res,next) => {
    var StartDate=add530(req.body.start)
    var StopDate=add530(req.body.stop)
