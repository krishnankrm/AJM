...
router.post('/MixerStatusReport', (req, res100, next) => __awaiter(this, void 0, void 0, function* () {
    ...
    MongoClient.connect(mongoclienturl, (err, db) => {
        if (!err) {
            var db0 = db.db("AJM");
            db0.collection('MixerStatus').aggregate([{ $match: { time: { $gte: StartDate, $lt: StopDate }, stoptime: { $ne: '' } } },
                { $project: { dateDifference: { $divide: [{ $subtract: ["$stoptime", "$time"] }, 1000] }, time: 1, stoptime: 1, cycleCount: 1 } }]).toArray((err, result) => __awaiter(this, void 0, void 0, function* () {
                if (!err) {
                    if (result.length != 0) {
                        let d = yield R(StartDate, StopDate);
                        excelOp23(result, d)
...
//# sourceMappingURL=reports%20Energy.js.map